...
export class TripView implements OnInit, OnDestroy, AfterViewInit {
            ...
            .subscribe((res) => {
                if (res) {
                    if (res.deleteId) {
                        ...
                        this._configurationService.updateAccountTrip(event.data.id, res)
                            .pipe(takeUntil(this.$destroy)).subscribe(() => {
                                this._getAllTrip();
                            });
                    }
    ...
    private _getAllTrip() {
        if (!this.generalPosition.includes(this.positionId)) {
            return this._configurationService.getAccountTrip(this.userId).subscribe((item: any) => {
            ...
            return this._configurationService.getAllAccountTrip().subscribe((item: any) => {
                this.accountTrip = item.message;
                this.accountTrip.forEach((data) => {
                    const findStatus = this.status.find(itemStatus => itemStatus.Id === data.stat);
                    const nameProject = this.projects.find(itemProject => itemProject.Id === data.prod);
