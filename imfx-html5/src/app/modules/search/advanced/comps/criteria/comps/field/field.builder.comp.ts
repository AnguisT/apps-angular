...
export class IMFXAdvancedCriteriaFieldBuilderComponent {
    // new implementation
    @ViewChild('advancedSearchCriteriaField') private fieldsEl: IMFXControlsSelect2Component;
    ...
    select(id: string | number, isManually: boolean = true) {
        ...
        this.onSelect(isManually);
    }

    /**
     * On select field
     */
    onSelect(isManually: boolean = true) {
        let fieldProp = this.common.props[this.fieldsEl.getSelected()];
        this.transfer.onSelectField(fieldProp);
