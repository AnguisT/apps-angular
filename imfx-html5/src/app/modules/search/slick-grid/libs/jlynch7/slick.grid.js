...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        function measureScrollbar() {
            var $c = $("<div style='position:absolute; top:-10000px; left:-10000px; width:100px; height:100px; overflow:scroll;'></div>").appendTo($container);
            var dim = {
                width: $c.width() - 1 - $c[0].clientWidth,
                height: $c.height() - 1 - $c[0].clientHeight
            };
            $c.remove();
            return dim;
        }
        ...
        function applyFullWidthRows(){
            var devideWidth, colWithWidth = 0, occupyWidth = 0,
            ...
            devideWidth = viewportW - vpSL - vpSR - occupyWidth;

            if (devideWidth > 0){
                devideWidth = Math.ceil(devideWidth / (columnsForFullWidth.length - colWithWidth));
                var colArr = $.extend(true, [], columnsForFullWidth);


                for (var i = 0; i < colArr.length; i++) {
                    if (typeof colArr[i].width != 'number'){
                        colArr[i].width = devideWidth;
                    }
                }

                // $.extend(true,columns, columnsForFullWidth);

                var reRender = false;
                for (i = 0; i < columns.length; i++) {
                    if (columns[i].rerenderOnResize && columns[i].width != colArr[i].width) {
                        reRender = true;
                    }
                    columns[i].width = colArr[i].width;
                }
            ...
            $.each(sortColumns, function (i, col) {
                ...
                if (columnIndex != null) {
                    headerColumnEls.eq(columnIndex).addClass("slick-header-column-sorted").find(".slick-sort-indicator").addClass(
                        col.sortAsc ? "slick-sort-indicator-asc icons-down icon" : "slick-sort-indicator-desc icons-up icon");
                }
            });
        }
        ...
        function bindMutationObserver(){
            ...
            if (typeof container == 'string'){
                ...
                if ((container[0])  && (container[0] instanceof Node)){
                    target = container[0];
                }
            ...
            observer = new MutationObserver(function(mutations) {
                mutations.forEach(contentFitInRowsChecking);
            });
        ...
        function contentFitInRowsChecking(mutation){
            ...
            for (var key in rowsCache){
                ...
                for (var i = 0; i< rowsCache[key].rowNode.length; i++) {
                    if (mutation instanceof MutationRecord){
                        isContain = (rowsCache[key].rowNode[i].contains(mutation.target)) ? true: false;
                    } else{
                        isContain = true;
                    }
                ...
                if (isContain) {
                    ...
                    locDiff = rowColumns.reduce(function (oldDiff, child) {
                        var curDiff = child.scrollHeight - child.clientHeight;
                        if (curDiff > oldDiff) {
                            return curDiff;
                        }
                        return oldDiff;
            ...
            observer && observer.observePrevious();
        }
