(function() {
  ...
  videojs.plugin('thumbnails', function(options) {
   ...
   // extend(img.style, settings['0'].style);
    ...
    moveFullWithThumb = function(left, imgFullWidth, progressBarWidth){
...
//      mouseTime = Math.floor((left - progressControl.el().offsetLeft) / progressControl.width() * duration);
      ...
      div.style.left = (progressControl.dragging ? right/2 : left) + 'px';
      if(progressControl.dragging) {
        ...
        moveFullWithThumb(leftCursorPosition, fwThumbImg.width, right);
      }
    };
    ...
    if (!player.multiSegments) {
      progressControl.on('mousedown', dragStartListener);
      progressControl.on('mouseup', dragStopListener);
