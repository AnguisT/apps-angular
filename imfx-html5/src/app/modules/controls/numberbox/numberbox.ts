...
export class IMFXControlsNumberboxComponent {
    @ViewChild('imfxNumberBoxModule') compRef;
    ...
    private validate($event) {
        ...
        if ($event.keyCode != 8 && $event.keyCode != 46) {
            ...
            if (allowedKeys.indexOf($event.keyCode) == -1 || !this.validateType($event)) {
    ...
    getValue(): NumberBoxValues {
        let val = this.prepare(this.compRef.nativeElement.value)||0;
        this.value = {
    ...
    keyUp($event) {
        setTimeout(() => {
            ...
            this.setValue(this.getValue().valueAsNumber);
    ...
    private prepare(val: string): number {
        val = val.replace(",", '.');
        if(val[val.length-1] == '.'){
            val += '0';
        }
