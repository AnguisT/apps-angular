...
import {
    GridStackOptions, SimplifiedSettings, SimplifiedSettingsDynamicWidgetItem,
...
@Injectable()
export class SimplifiedSettingsProvider {
    public settings: SimplifiedSettings;
    public tmpSettings: SimplifiedSettings;
    ...
    public transfer: SimplifiedSettingsTransferProvider;
    private idOfLastDroppedElement: string;

    getDefaultItemSettings(): SimplifiedSettings {
        return {
            'dynamicWidgets': [],
            'staticWidgets': {
                'cart': {
    ...
    getDefaultDetailSettings(): SimplifiedSettings {
        return {
            'dynamicWidgets': [],
            'staticWidgets': {
                'ThumbUrl': {
    ...
    getDefaultSettings(type: 'item' | 'detail' = 'item'): SimplifiedSettings {
        let res: SimplifiedSettings;
        if (type === 'item') {
    ...
    save(settingsGroupId) {
        this.moduleContext.gridStack.savePanel().subscribe((items) => {
            ...
            this.transfer.updated.emit(<TransferdSimplifedType>{
                setupType: 'simplified' + '.' + this.moduleContext.type,
     ...
     * @param type
     */
    reset(type, newSetups?: SimplifiedSettings) {
     ...
     * @param newSetups
     */
    setSetups(type, newSetups: SimplifiedSettings) {
    ...
    addLabel() {
        ...
        modal.modalEvents.subscribe((e: IMFXModalEvent) => {
            if (e.name === 'ok') {
                ...
                let labelWidget: SimplifiedSettingsDynamicWidgetItem = {
                    id: id,
                    value: label,
                    enabled: true,
                    x: 0,
    ...
    /**
     * UNUSED
     * Get all defs and apply it for module
     * @param id
     * @param type
     * @returns {any}
     */
    // getAndApplySettingsForCurrentGroup(id: number, type: 'item' | 'detail' = 'item') {
    //     return Observable.create((observer) => {
    //         return this.service.getSettingsGroupById(id).subscribe((setups) => {
    //             let s: SimplifiedSettings;
    //             $.each(setups.TM_SETTINGS_KEYS, (k, o) => {
    //                 if (o.KEY == 'simplified') {
    //                     if (type == 'item') {
    //                         s = JSON.parse(o.DATA).SimplifiedItemLayout;
    //                     } else if (type == 'detail') {
    //                         s = JSON.parse(o.DATA).SimplifiedDetailLayout;
    //                     }
    //                     return false
    //                 }
    //             });
    //             this.settings = $.extend(true, this.getDefaultSettings(type), s);
    ...
    getFields(): Observable<Subscription> {
        return Observable.create((observer) => {
            this.service.getFieldsForSimplified().subscribe((fields: any) => {
    ...
    openListOfFiledsModal() {
        this.fieldsModal = this.moduleContext.modalProvider.show(SettingsSimplifiedFieldsComponent, {
            size: 'sm',
            title: 'simplified.field_setup',
            top: '47%',
            isFooter: false,
            height: '70vh',
            position: 'center',
    ...
    private applyAndStoreSettings(settings: SimplifiedSettings) {
