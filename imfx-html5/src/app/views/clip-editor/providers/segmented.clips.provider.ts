...
export class SegmentetClipsProvider {
  ...
  public getClips(clips: Array<any>) {
    if (this.componentRef.src instanceof Array) {
      let duration = this.componentRef.gl.playerComponents.compRef.instance.player.duration();
      ...
      for (let clip of clipsWithSeconds) {
        ...
        let startOwnerIdx = this.getMultiSourceOwnerIndexBySeconds(<RCEArraySource>this.componentRef.src, clip.in);
        let endOwnerIdx = this.getMultiSourceOwnerIndexBySeconds(<RCEArraySource>this.componentRef.src, clip.out);
