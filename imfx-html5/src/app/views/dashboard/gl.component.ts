...
export class GLDashboardComponent {
  ...
  ngOnInit() {
    ...
    this.serverStorage.retrieve([self.storagePrefix], true).subscribe((result) => {
      ...
      self.settingsGroupsService.getSettingsGroupById(0).subscribe((res) => {
        if(res) {
          let data = {
            "ID": res.ID,
            "NAME": res.NAME,
          ...
          if(grafanaSettings) {
            ...
            if(grafanaDataArray.length > 0) {
              ...
              for(var k=0; k<grafanaDataArray.length; k++) {
  ...
  addItemMenuDragSource() {
    var elementChart = $('#chart-drag');
    var elementWFBoard = $('#wfboard-drag');
    var elementGrafana = $('#grafana-drag');
  ...
  setView() {
    ...
    self.layout.on('stateChanged', function () {
      ...
      self.serverStorage.store(self.storagePrefix, state).subscribe((res) => {
        console.log("saved");
      });
    });

    self.layout.on( 'tabCreated', function( tab ){
      // var settingsCtrl = $( $( '#template-control' ).html() );
      // var settingsBtn = settingsCtrl.find( '.settingsButton' );
      // setTimeout(function(){
      //   var container = tab.contentItem.parent.getActiveContentItem().container;
      //   var state = container.getState();
      //   if(state.firstDrop_hiddenControl) {
      //     var disableFirstShowing = {firstDrop_hiddenControl: false};
  ...
  initHeaderControls(stack, self) {
    ...
    var setState = function(saveBtn){
      var newState = {}
      var attributesContainer = $(saveBtn);
      var attributes = attributesContainer.parent().parent().parent().find('.control-wrapper');
      for(var i = 0; i < attributes.length; i++) {
        var valueInput = $(attributes[i]).find('input');
        var key = $(attributes[i]).attr("data-key");
        var value;
        if(valueInput.length > 1) {
          value = [];
          for(var j = 0; j < valueInput.length; j++) {
          ...
          var valueSelect = $(attributes[i]).find('option:selected');
          var selected = {};
    ...
    var prepareSettingsContainer = function() {
      ...
      var container = stack.getActiveContentItem().container;
      var state = container.getState();
      var settingsContainerHtml = "<div class='settings-wrapper'><div class='controls-wrapper'>";
      var settingsControlsHtml = "";
        ...
        if(key.indexOf('_hiddenControl') == -1) {
          ...
          if (type == "[object Object]") {
            if (value.choises && value.value && value.choises.length > 0) {
              ...
              for (var i = 0; i < value.choises.length; i++) {
                var realData = ""
      ...
      var saveBtn = $(stack.header.controlsContainer).find('.save-btn');
